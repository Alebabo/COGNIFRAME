...
2. **Streams**: exactly one video and one audio stream; audio is not silent across any segment.
...
- Never approve a check you could not measure — report `unmeasurable` with the reason instead of `pass`.
- Output is JSON only.
